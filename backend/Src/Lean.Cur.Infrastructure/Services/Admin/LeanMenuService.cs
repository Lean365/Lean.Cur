...
public class LeanMenuService : ILeanMenuService
{
    ...
    public async Task<LeanMenuImportResultDto> ImportAsync(IFormFile file)
    {
        ...
        var importDtos = await _excel.ImportAsync<LeanMenuImportDto>(file);
    ...
    public async Task<byte[]> ExportAsync(LeanMenuQueryDto queryDto)
    {
        ...
        {
            query = query.Where(m => m.MenuName.Contains(queryDto.MenuName));
        }
        if (!string.IsNullOrEmpty(queryDto.MenuCode))
        {
            query = query.Where(m => m.MenuCode.Contains(queryDto.MenuCode));
        }
        if (queryDto.MenuType.HasValue)
        {
            query = query.Where(m => m.MenuType == queryDto.MenuType.Value);
        }
        ...
        {
            query = query.Where(m => m.CreateTime >= queryDto.StartTime.Value);
        }
        if (queryDto.EndTime.HasValue)
        {
            query = query.Where(m => m.CreateTime <= queryDto.EndTime.Value);
        }

        var list = await query.Select(m => new LeanMenuExportDto
        {
            ParentMenuName = SqlFunc.Subqueryable<LeanMenu>()
                .Where(p => p.Id == m.ParentId)
                .Select(p => p.MenuName),
            MenuName = m.MenuName,
            MenuCode = m.MenuCode,
            ...
            Permission = m.Permission,
            Icon = m.Icon,
            OrderNum = m.OrderNum.ToString(),
            Status = m.Status.ToString(),
            Visible = m.Visible == 1 ? "是" : "否",
            Cache = m.Cache == 1 ? "是" : "否",
            External = m.External == 1 ? "是" : "否",
            Remark = m.Remark,
            CreateTime = m.CreateTime.ToString("yyyy-MM-dd HH:mm:ss"),
            UpdateTime = m.UpdateTime.HasValue ? m.UpdateTime.Value.ToString("yyyy-MM-dd HH:mm:ss") : string.Empty
        }).ToListAsync();
