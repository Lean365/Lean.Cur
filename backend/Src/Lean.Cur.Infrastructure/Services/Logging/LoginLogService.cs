...
namespace Lean.Cur.Infrastructure.Services.Logging
{
  ...
  public class LoginLogService : ILoginLogService
  {
    ...
    public LoginLogService(SqlSugarClient db, ILogger<LoginLogService> logger, LeanExcelHelper excel)
    {
      _db = db;
      _logger = logger;
      _excel = excel;
    ...
    /// <inheritdoc/>
    public async Task<LeanPagedResult<LoginLogDto>> GetPageAsync(LoginLogQueryDto queryDto)
    {
      try
      {
        ...
        return new LeanPagedResult<LoginLogDto>(list, total, queryDto.PageIndex, queryDto.PageSize);
      }
      catch (Exception ex)
      {
