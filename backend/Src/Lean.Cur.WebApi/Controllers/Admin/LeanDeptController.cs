...
public class LeanDeptController : LeanBaseController
{
  ...
  [HttpGet("list")]
  public async Task<LeanApiResult<List<LeanDeptDto>>> GetListAsync([FromQuery] LeanDeptQueryDto queryDto)
  {
  ...
  [HttpGet("{id}")]
  public async Task<LeanApiResult<LeanDeptDto>> GetByIdAsync(long id)
  {
    if (id <= 0)
    {
  ...
  [HttpPost]
  public async Task<LeanApiResult<LeanDeptDto>> CreateAsync(LeanDeptCreateDto createDto)
  {
  ...
  /// <returns>部门信息</returns>
  [HttpPut]
  public async Task<LeanApiResult<LeanDeptDto>> UpdateAsync(LeanDeptUpdateDto updateDto)
  {
    if (updateDto.Id <= 0)
    {
      return ValidateError<LeanDeptDto>("部门ID必须大于0");
  ...
  /// <returns>是否成功</returns>
  [HttpPut("status")]
  public async Task<LeanApiResult<bool>> UpdateStatusAsync(LeanDeptStatusDto statusDto)
  {
    if (statusDto.Id <= 0)
    {
      return ValidateError<bool>("部门ID必须大于0");
  ...
  [HttpPost("import")]
  public async Task<LeanApiResult<LeanDeptImportResultDto>> ImportAsync(IFormFile file)
  {
