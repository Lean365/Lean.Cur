...
public class LeanRoleController : LeanBaseController
{
  ...
  [HttpGet("page")]
  public async Task<LeanApiResult<LeanPagedResult<LeanRoleDto>>> GetPagedListAsync([FromQuery] LeanRoleQueryDto queryDto)
  {
  ...
  [HttpGet("{id}")]
  public async Task<LeanApiResult<LeanRoleDto>> GetByIdAsync(long id)
  {
    if (id <= 0)
    {
  ...
  [HttpPost]
  public async Task<LeanApiResult<long>> CreateAsync(LeanRoleCreateDto createDto)
  {
  ...
  [HttpPut]
  public async Task<LeanApiResult<bool>> UpdateAsync(LeanRoleUpdateDto updateDto)
  {
    if (updateDto.Id <= 0)
    {
  ...
  /// <returns>是否成功</returns>
  [HttpPut("status")]
  public async Task<LeanApiResult<bool>> UpdateStatusAsync(LeanRoleStatusDto statusDto)
  {
    if (statusDto.Id <= 0)
    {
      return ValidateError<bool>("角色ID必须大于0");
  ...
  [HttpPost("import")]
  public async Task<LeanApiResult<LeanRoleImportResultDto>> ImportAsync(IFormFile file)
  {
  ...
  public async Task<LeanApiResult<List<string>>> GetRolePermissionsAsync(long roleId)
  {
    if (roleId <= 0)
    {
  ...
  /// <returns>是否成功</returns>
  [HttpPut("permissions")]
  public async Task<LeanApiResult<bool>> UpdateRolePermissionsAsync(LeanRoleMenuPermissionDto permissionDto)
  {
  ...
  [HttpGet("users")]
  public async Task<LeanApiResult<LeanPagedResult<LeanRoleUserListDto>>> GetRoleUsersAsync([FromQuery] LeanRoleUserQueryDto queryDto)
  {
    if (queryDto.RoleId <= 0)
    {
      return ValidateError<LeanPagedResult<LeanRoleUserListDto>>("角色ID必须大于0");
    }
  ...
  /// <returns>是否成功</returns>
  [HttpPost("users")]
  public async Task<LeanApiResult<bool>> AssignUsersAsync(LeanRoleUserAssignDto assignDto)
  {
