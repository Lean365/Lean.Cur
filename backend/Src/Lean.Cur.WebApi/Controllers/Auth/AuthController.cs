...
public class AuthController : LeanBaseController
{
  ...
  [HttpPost("login")]
  [AllowAnonymous]
  public async Task<LeanApiResult<LoginResultDto>> LoginAsync([FromBody] LoginDto loginDto)
  {
  ...
  [HttpPost("logout")]
  [Authorize]
  public async Task<LeanApiResult<bool>> LogoutAsync()
  {
  ...
  [AllowAnonymous]
  public async Task<LeanApiResult<LoginResultDto>> RefreshTokenAsync([FromBody] string refreshToken)
  {
  ...
  [HttpGet("verify-code")]
  [AllowAnonymous]
  public async Task<LeanApiResult<LeanSlideVerifyResult>> GenerateVerifyCodeAsync()
  {
  ...
  [HttpGet("user-info")]
  [Authorize]
  public async Task<LeanApiResult<LoginUserDto>> GetUserInfoAsync()
  {
    var userId = Convert.ToInt64(User.Claims.FirstOrDefault(x => x.Type == "userId")?.Value);
    if (userId <= 0)
    {
