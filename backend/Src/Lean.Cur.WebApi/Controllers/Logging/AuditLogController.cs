...
namespace Lean.Cur.WebApi.Controllers.Logging
{
  ...
  public class AuditLogController : LeanBaseController
  {
    ...
    [HttpGet("page")]
    public async Task<LeanApiResult<LeanPagedResult<AuditLogDto>>> GetPageAsync([FromQuery] AuditLogQueryDto queryDto)
    {
      if (!ModelState.IsValid)
      {
        return ValidateError<LeanPagedResult<AuditLogDto>>("请求参数验证失败");
      }
    ...
    [HttpGet("{id}")]
    public async Task<LeanApiResult<AuditLogDto>> GetAsync(long id)
    {
      if (id <= 0)
      {
    ...
    [HttpDelete("clear")]
    public async Task<LeanApiResult<int>> ClearAsync(DateTime beforeTime)
    {
    ...
    [HttpGet("stats")]
    public async Task<LeanApiResult<AuditLogStatsDto>> GetStatsAsync(DateTime? startTime = null, DateTime? endTime = null)
    {
    ...
    [HttpGet("trend")]
    public async Task<LeanApiResult<List<AuditLogTrendDto>>> GetTrendAsync(int days = 7)
    {
      if (days <= 0 || days > 90)
      {
